...
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO, "ncnn_llm_gpt", "%s", s.c_str());
#else
    std::cerr << s << std::endl;
#endif
}

static void log_warn(const std::string& s) {
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_WARN, "ncnn_llm_gpt", "%s", s.c_str());
#else
    std::cerr << s << std::endl;
#endif
}

static std::string mem_summary() {
#ifdef __ANDROID__
    struct sysinfo si;
    if (sysinfo(&si) != 0) return "mem=unknown";
    const unsigned long long unit = (unsigned long long)si.mem_unit;
    unsigned long long total = (unsigned long long)si.totalram * unit;
    unsigned long long free = (unsigned long long)si.freeram * unit;
    unsigned long long avail = (unsigned long long)si.freeram * unit;
    std::ostringstream oss;
    oss << "mem_total=" << total << " mem_free=" << free << " mem_avail~=" << avail;
    return oss.str();
#else
    return "mem=unknown";
#endif
...
static long long file_size_safe(const std::string& path) {
    try {
        return (long long)std::filesystem::file_size(std::filesystem::path(path));
    } catch (...) {
        return -1;
    }
}

static void require_ok(int ret, const char* what, const std::string& path) {
    if (ret == 0) return;

    // Best-effort diagnostics for why load_param/load_model failed.
    int open_errno = 0;
    std::string open_errstr;
    {
        std::ifstream ifs(path, std::ios::binary);
        if (!ifs.good()) {
            open_errno = errno;
            open_errstr = std::strerror(open_errno);
        }
    }
    if (open_errno != 0) {
        log_warn(std::string("Open failed before ") + what + ": path=" + path + " errno=" +
                 std::to_string(open_errno) + " err=" + open_errstr);
    } else {
        log_info(std::string("Open ok before ") + what + ": path=" + path + " size=" + std::to_string(file_size_safe(path)));
    }
    log_info(std::string("Before ") + what + ": " + mem_summary());
#if INTPTR_MAX == INT32_MAX
    log_warn("Process is 32-bit (possible address-space limits for large models)");
#endif

    std::ostringstream oss;
    oss << what << " failed ret=" << ret << " path=" << path << " size=" << file_size_safe(path);
...
ncnn_llm_gpt::ncnn_llm_gpt(const std::string& model_path, bool use_vulkan) {
    try {
        ...
        // Be conservative on Android/emulator stability by default.
        decoder_net->opt.num_threads = 1;
        embed_net->opt.num_threads = 1;
        proj_out_net->opt.num_threads = 1;
        ...
        require_ok(decoder_net->load_param(decoder_param.c_str()), "decoder load_param", decoder_param);
        require_ok(decoder_net->load_model(decoder_bin.c_str()), "decoder load_model", decoder_bin);
        require_ok(embed_net->load_param(embed_param.c_str()), "embed load_param", embed_param);
        require_ok(embed_net->load_model(embed_bin.c_str()), "embed load_model", embed_bin);
        require_ok(proj_out_net->load_param(proj_out_param.c_str()), "proj_out load_param", proj_out_param);
        require_ok(proj_out_net->load_model(proj_out_bin.c_str()), "proj_out load_model", proj_out_bin);
        ...
        if (config["setting"].contains("vision")) {
            ...
            if (vision_type != "close") {
                ...
                vision_embed_patch->opt.num_threads = 1;
                vision_encoder->opt.num_threads = 1;

                if (use_vulkan) {
                    vision_embed_patch->opt.use_vulkan_compute = true;
                    vision_encoder->opt.use_vulkan_compute = true;
                }
                require_ok(vision_embed_patch->load_param(vision_embed_patch_param.c_str()), "vision_embed_patch load_param", vision_embed_patch_param);
                require_ok(vision_embed_patch->load_model(vision_embed_patch_bin.c_str()), "vision_embed_patch load_model", vision_embed_patch_bin);
                require_ok(vision_encoder->load_param(vision_encoder_param.c_str()), "vision_encoder load_param", vision_encoder_param);
                require_ok(vision_encoder->load_model(vision_encoder_bin.c_str()), "vision_encoder load_model", vision_encoder_bin);
