...
static void mcplog_i(const std::string& s) { __android_log_print(ANDROID_LOG_INFO, "ncnn_llm_mcp", "%s", s.c_str()); }
static void mcplog_w(const std::string& s) { __android_log_print(ANDROID_LOG_WARN, "ncnn_llm_mcp", "%s", s.c_str()); }
#else
static void mcplog_i(const std::string& s) { std::cerr << s << "\n"; }
static void mcplog_w(const std::string& s) { std::cerr << s << "\n"; }
#endif
...
struct McpStdioClient::Impl {
    ...
    json call_tool(const std::string& name, const json& arguments, std::string* err) {
        mcplog_i("MCP tools/call name=" + name + " args=" + truncate_for_log(arguments.dump(), 800));
        json params = {{"name", name}, {"arguments", arguments}};
        json out = request("tools/call", params, err);
        if (err && !err->empty()) {
            mcplog_w("MCP tools/call failed name=" + name + " err=" + truncate_for_log(*err, 800));
        } else {
            mcplog_i("MCP tools/call ok name=" + name + " result=" + truncate_for_log(out.is_null() ? "null" : out.dump(), 800));
        }
        return out;
    }
...
private:
    ...
    bool send(const json& msg) {
        ...
        if (debug) {
            ...
            mcplog_i(std::string("[MCP] => ") + preview);
        }
