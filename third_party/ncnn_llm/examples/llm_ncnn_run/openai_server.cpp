...
int run_openai_server(const Options& opt,
    ...
    server.Post("/v1/chat/completions", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        if (!tools.empty()) {
            ...
            cfg.tool_callback = [&, mcp_client, allowed, max_tool_string_bytes, artifacts_out, artifacts_seen, tool_trace, mcp_image_delivery](
        ...
        if (stream) {
            ...
            res.set_chunked_content_provider(
                "text/event-stream",
                [&, prompt, cfg, resp_id, model_name, artifacts_out, tool_trace](size_t, httplib::DataSink& sink) mutable {
